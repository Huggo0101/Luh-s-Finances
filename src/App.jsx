          ...
          <div className={`bg-white p-6 md:p-8 rounded-3xl shadow-xl border h-fit ${editandoId ? 'border-amber-400 ring-4 ring-amber-50' : 'border-gray-100'}`}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="flex gap-2">
                ...
                <input required type="text" placeholder={placeholders[tipo]} value={descricao} onChange={(e) => setDescricao(e.target.value)} className="w-2/3 p-3 rounded-2xl border border-gray-100 bg-gray-50 outline-none font-medium" />
              </div>
            ...
            {dadosGrafico.length > 0 ? (
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  ...
                  <Legend verticalAlign="bottom" height={36} iconType="circle" wrapperStyle={{ paddingTop: '20px', fontSize: '10px', fontBold: '900', textTransform: 'uppercase' }} />
                </PieChart>
              </ResponsiveContainer>
            ) : (
